...
class Generator(nn.Module):
    def __init__(self, output_shape):
        ...
        # LATENT_VECTOR_SIZE is input vector
...
def iterate_batches(envs, batch_size=BATCH_SIZE):
    ...
    while True:
        e = next(env_gen)
        res = e.step(e.action_space.sample())
...
if __name__ == "__main__":
    ...
    for batch_v in iterate_batches(envs):
        ...
        # train generator，生成器目标是想让判别器无法判别
        ...
        if iter_no % REPORT_EVERY_ITER == 0:
            log.info(
                f"Iter {iter_no}: gen_loss={np.mean(gen_losses)}, dis_loss={np.mean(dis_losses)}"
            )
